...
export const QuizGenerator = ({ lectureContent }: QuizProps) => {
    ...
    return (
        <div className="p-5 bg-gray-900 border border-gray-700 rounded-lg shadow-lg">
            <h3 className="text-xl font-bold mb-4 text-gray-100">Generate Quiz</h3>
            <Button
            ...
            {quiz.length > 0 && (
                <div className="p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-lg my-4">
                    <h3 className="text-lg font-bold text-purple-400">Time to take the quiz!</h3>
                                ...
                                >
                                    {q.choices.map((choice, i) => (
                                        <div key={i} className="flex items-center space-x-2">
                                            ...
                                            />
                                            <Label htmlFor={`q-${index}-choice-${i}`}>
                                                {choice}
                                            </Label>
                                        </div>
                                    ))}
                    ...
                    {score === quiz.length ? (
                        <p className="mt-2 text-green-400">Congratulations! You got all answers correct!</p>
                    ) : (
                        <p className="mt-2 text-green-400">Better luck next time!</p>
                    )}
                </div>
            )}
        </div>
